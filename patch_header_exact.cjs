...
const targetHeaderRegex = /const Header: React\.FC<\{ onMenuClick: \(\) => void, className\?: string \}> = \(\{ onMenuClick, className \}\) => \{[\s\S]*?<\/header>\n \);\n\};/;
...
if (targetHeaderRegex.test(code)) {
    code = code.replace(targetHeaderRegex, newHeader);
    fs.writeFileSync('src/AppCore.tsx', code);
    console.log("Header replaced properly.");
} else {
    console.log("Could not match Header regex.");
}
